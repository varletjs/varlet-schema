...
const FUNCTIONS = ['ImagePreview', 'Snackbar', 'Picker', 'ActionSheet', 'Dialog', 'Locale', 'StyleProvider', 'LoadingBar']
...
export function useVarlet() {
  ...
  const components = shallowRef(exports.reduce((components, [key, value]) => {
  ...
  const injects = shallowRef(exports.reduce((injects, [key, value]) => {
